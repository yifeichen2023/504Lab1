...
`r qbegin(qcounter())` Below is some code that fits a model of "reading
...
`r qend()` `r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
...
`r length(unique(efrep$PID))` healthy volunteers each completed the
...
`r qbegin(qcounter())` How many participants are there in the data?\
...
`r qbegin(qcounter())` Make factors and set the reference levels of the
...
`r qbegin(qcounter())` Fit a multilevel model to address the aims of the
study (copied below)
...
-   3 different groups of audio type
    (`r paste(unique(efdat$audio),collapse=", ")`)
-   2 groups of listening condition
    (`r paste(unique(efdat$headphones),collapse=", ")`)
...
`r qbegin(qcounter())` We now have a model, but we don't have any
...
`r qbegin(qcounter())` Now we have some p-values and a plot, try to
...
on average `r res[1,2]` on the SDMT. For participants without
headphones, listening to music via speakers was associated with lower
scores compared to no audio
($b = `r res[2,2]`, t(`r res[2,8]`)=`r res[2,7]`, p `r res[2,9]`$), but
...
($b = `r res[4,2]`, t(`r res[4,8]`)=`r res[4,7]`, p `r res[4,9]`$). The
...
($b = `r res[5,2]`, t(`r res[5,8]`)=`r res[5,7]`, p `r res[5,9]`$).
...
ANC-heaphones
($b = `r res[6,2]`, t(`r res[6,8]`)=`r res[6,7]`, p `r res[6,9]`$).

There was considerable variability in baseline (i.e. no-audio) SDMT
scores across participants (SD = `r res2[1,5]`), with participants
showing similar variability in the effects of music (SD = `r res2[2,5]`)
and of white-noise (SD = `r res2[3,5]`). A weak negative correlation
(`r res2[5,5]`) between participant-level intercepts and effects of
...
weak negative correlation (`r res2[6,5]`) between music and white-noise
